...
object `package` {
  ...
  def fromScope(scope: NamespaceBinding): List[(Option[String], String)] = {
    def doFromScope(s: NamespaceBinding): List[(Option[String], String)] = {
      ...
      IPXACT2009scalaxb.Helper.nullOrEmpty(s.uri) map { uri => (IPXACT2009scalaxb.Helper.nullOrEmpty(s.prefix) -> uri) :: parentMap } getOrElse {parentMap}
    }
...
trait XMLStandardTypes {
  ...
  implicit def seqXMLFormat[A: XMLFormat]: XMLFormat[Seq[A]] = new XMLFormat[Seq[A]] {
    ...
    def writes(obj: Seq[A], namespace: Option[String], elementLabel: Option[String],
        scope: scala.xml.NamespaceBinding, typeAttribute: Boolean): scala.xml.NodeSeq =
      Helper.stringToXML((obj map { x => IPXACT2009scalaxb.toXML(x, namespace, elementLabel, scope, typeAttribute).text }).mkString(" "),
  ...
  implicit def dataRecordFormat[A: XMLFormat]: XMLFormat[DataRecord[A]] = new XMLFormat[DataRecord[A]] {
    def reads(seq: scala.xml.NodeSeq, stack: List[ElemName]): Either[String, DataRecord[A]] = seq match {
      case node: Node =>
        try {
          Right(DataRecord(Some(node.namespace), Some(node.label), IPXACT2009scalaxb.fromXML[A](node)))
        } catch { case e: Exception => Left(e.toString) }
      case _ => Left("scala.xml.Node is required.")
    }
  ...
  implicit def someXMLWriter[A: CanWriteXML]: CanWriteXML[Some[A]] = new CanWriteXML[Some[A]] {
    def writes(obj: Some[A], namespace: Option[String], elementLabel: Option[String],
        scope: scala.xml.NamespaceBinding, typeAttribute: Boolean): scala.xml.NodeSeq =
      IPXACT2009scalaxb.toXML[A](obj.get, namespace, elementLabel, scope, typeAttribute)
  }
  ...
  implicit def optionXMLWriter[A: CanWriteXML]: CanWriteXML[Option[A]] = new CanWriteXML[Option[A]] {
    def writes(obj: Option[A], namespace: Option[String], elementLabel: Option[String],
      ...
      case Some(x) => IPXACT2009scalaxb.toXML[A](x, namespace, elementLabel, scope, typeAttribute)
  ...
  implicit lazy val __DataRecordMapWriter: CanWriteXML[Map[String, IPXACT2009scalaxb.DataRecord[Any]]] =
    new CanWriteXML[Map[String, IPXACT2009scalaxb.DataRecord[Any]]] {
      def writes(obj: Map[String, IPXACT2009scalaxb.DataRecord[Any]], namespace: Option[String], elementLabel: Option[String],
          scope: scala.xml.NamespaceBinding, typeAttribute: Boolean): scala.xml.NodeSeq =
        obj.valuesIterator.toList flatMap { x =>
          IPXACT2009scalaxb.toXML[DataRecord[Any]](x, x.namespace, x.key, scope, typeAttribute)
        }
    }
}
...
object DataRecord extends XMLStandardTypes {
  ...
  def apply[A:CanWriteXML](node: Node, value: A): DataRecord[A] = node match {
    case elem: Elem =>
      val ns = IPXACT2009scalaxb.Helper.nullOrEmpty(elem.scope.getURI(elem.prefix))
...
object ElemName {
  implicit def apply(node: scala.xml.Node): ElemName = node match {
    case x: scala.xml.Elem =>
      val elemName = ElemName(IPXACT2009scalaxb.Helper.nullOrEmpty(x.scope.getURI(x.prefix)), x.label)
      elemName.node = x
      elemName
    case _ =>
...
trait AttributeGroupFormat[A] extends IPXACT2009scalaxb.XMLFormat[A] {
...
object Helper {
  ...
  def instanceType(node: scala.xml.Node): (Option[String], Option[String]) = {
    ...
    val namespace = IPXACT2009scalaxb.Helper.nullOrEmpty(node.scope.getURI(prefix.orNull))
    ...
    node match {
      case elem: Elem =>
        withInnerScope(elem.scope, outer) { (innerScope, mapping) =>
          val newPrefix: String = mapping.get(IPXACT2009scalaxb.Helper.nullOrEmpty(elem.prefix)) map {_.orNull} getOrElse {elem.prefix}
